...
print "[iptvDream] dreambox timezone is GMT", Timezone
...
print "[iptvDream] resetting time delta !!!"
time_delta = secTd(0)
	
def setSyncTime(time):
	global time_delta
	time_delta = time-datetime.datetime.now()
	print "[iptvDream] set time delta to", tdSec(time_delta)

def syncTime():
	#print "[iptvDream] time delta = ", tdSec(time_delta)
...
class Channel(object):
	...
	def findEpg(self, time):
		...
		while (i < len(self.q)) and not self.q[i].isNow(time):
			i += 1
		if i == len(self.q):
			#print "[iptvDream] epg not found for", time
			return None
		else:
			return i
...
class Bouquet():
	...
	def sortByKey(self, keyn):
		...
		print "[iptvDream] sorting", self.name, keyn
		if keyn == 1:
...
class BouquetManager():
	...
	def goIn(self, index=None):
		...
		if self.current.type == Bouquet.TYPE_MENU:
			...
			self.current.index = 0
		print "[iptvDream] bouquet In", self.current.name, self.current.index
		#return self.getList()
	...
	def goOut(self):
		if self.current.parent:
			...
			print "[iptvDream] bouquet Out", self.current.name, self.current.index
		#return self.getList()
	...
	def setPath(self, path, cid):
		...
		if self.getCurrent() != cid:
			print "[iptvDream] service not found in path!"
			self.current = self.root
			return False
		return True
	
	def historyNext(self):
		#print "[iptvDream]", self.history, self.historyId
	...
	def historyPrev(self):
		...
		if self.historyId > 0:
			self.historyId -= 1
			print "[iptvDream]", self.historyId
			h = self.history[self.historyId]
			self.current = self.root
			return self.setPath(h[0], h[1])
		else: return False
