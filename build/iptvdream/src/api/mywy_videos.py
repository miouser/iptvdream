...
class Ktv(MyWyAPI):
	...
	def getVideos(self, stype='last', page=1, genre=[],  nums_onpage=20, query=''):
		...
		if stype == 'text':
			params['word'] = query
		root = self.getData("/get_list_movie?"+urllib.urlencode(params), "getting video list by type %s" % stype)
		options = root.find('options')
		videos_count = int(options.findtext('count'))
		
		self.currentPageIds = []
		for v in root.find('groups'):
			vid = int(v.findtext('id'))
			self.currentPageIds += [vid]
			name = v.findtext('title').encode('utf-8')
			video = Video(name)
#			video.name_orig = v.findtext('name_orig').encode('utf-8')
#			video.descr = unescapeEntities(v.findtext('description')).encode('utf-8')
			video.image = v.findtext('pic')
...
#			video.country = v.findtext('country').encode('utf-8')
			video.genre = v.findtext('genre').encode('utf-8')
...
#		self.videos[vid]= video
		...
		params = {"cid" : vid}
		root = self.getData("/get_url_movie?"+urllib.urlencode(params), "getting video url %s" % vid)
		print "play" % vid
	...
	def getVideoGenres(self):
		root = self.getData("/get_genre_movie?", "getting genres list")		
		self.video_genres = []
		for genre in root.find('groups'):
			self.video_genres += [{"id": genre.findtext('id'), "name": genre.findtext('title').encode('utf-8')}]
